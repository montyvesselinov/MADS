...
// function headers
void dream_zs( MCMC MCMCPar, Range ParRange, Measure Measurement, string ModelName, struct opt_data *od, int option,
			   double ***Reduced_Seq, int X, double **Zee, out &output, double **Zinit,
...
void LHSU( Range ParRange, MCMC MCMCPar, double **Zinit );
void comp_likelihood( double **x, MCMC MCMCPar, Measure Measurement, string ModelName, struct opt_data *od, int option, double *ModPred, double *p, double *log_p );
void InitVariables( MCMC &MCMCPar, int &nEval, int &output_teller, int &reduced_seq_teller, double **Zee,
					double **Table_JumpRate, double ***Reduced_Seq, int &iloc, out &output, double &flag,
					double ***Sequences, double **pCR, double **CRS, double **lCR, double **lCRnew );
void CalcCbWb( MCMC MCMCPar, double &Cb, double &Wb );
void GenCR( MCMC MCMCPar, double **lCR, double **pCR, double **CRS );
void multrnd( int nn, int m, int k, double **lCR, double **Y, double **pCR );
void Gelman( MCMC MCMCPar, double *R_stat, double ***Sequences, int start_loc, int end_loc );
void GelmanCum( MCMC MCMCPar, double *R_stat, double **X2 );
void GetLocation( double **X2, MCMC MCMCPar, double **xold, double *p_xold, double *log_p_xold );
void randsample( int n, int k, int *ro, int verbose );
void offde( double **xold, double **Zoff, MCMC MCMCPar, string Update, double **Table_JumpRate, Range ParRange, double **xnew,
			double *alpha_s, double **CRS, int **DEversion );
void DEStrategy( MCMC MCMCPar, int **DEversion );
...
void metrop( double **xnew, double *p_xnew, double *log_p_x, double **xold, double *p_xold, double *log_p_xold,
			 double *alpha_s, Measure Measurement, MCMC MCMCPar, double option, double **newgen, int *accept );
void CalcDelta( MCMC MCMCPar, double *delta_normX, double **CRS, double *delta_tot );
void AdaptpCR( MCMC MCMCPar, double *delta_tot, double **lCR, double **pCR );
...
//main rundream code
int setup_mcmc( struct opt_data *od )
{
	struct MCMC MCMCPar;
	...
	// -----------------------------------------------------------------------------------------------------------------------
	MCMCPar.ppCR = "Update";                   // Adaptive tuning of crossover values
	// -----------------------------------------------------------------------------------------------------------------------
	// --------------------------------------- Added for reduced sample storage ----------------------------------------------
	MCMCPar.reduced_sample_collection = "No"; // Thinned sample collection?
	...
	MCMCPar.InitPopulation = "LHS_BASED";
	// Define the boundary handling
	MCMCPar.BoundHandling = "Reflect";
	// Save in memory or not
	MCMCPar.save_in_memory = "No";
	MCMCPar.save_in_file = "No";
	...
	double *ModPred;
	ModPred = new double[Measurement.N];
	// make Zinit pointer to pass back and forth from functions
	double **Zinit;
	Zinit = double_matrix( MCMCPar.m0 + MCMCPar.seq, MCMCPar.n );
	// make 3dArray Sequences pointer to pass back and forth from functions
	double ***Sequences;
	if( strcmp( MCMCPar.save_in_memory.c_str(), "Yes" ) == 0 )
	{
		Sequences = new double **[MCMCPar.seq];
		for( int row = 0; row < MCMCPar.seq; row++ )
		{
			Sequences[row] = new double* [MCMCPar.seq_length];
			for( int col = 0; col < MCMCPar.seq_length; col++ )
				Sequences[row][col] = new double[MCMCPar.np2];
		}
	}
	// make 3dArray Sequences pointer to pass back and forth from functions
	double ***Reduced_Seq;
	if( strcmp( MCMCPar.reduced_sample_collection.c_str(), "Yes" ) == 0 )
	{
		Reduced_Seq = new double **[MCMCPar.seq];
		for( int row = 0; row < MCMCPar.seq; row++ )
		{
			Reduced_Seq[row] = new double* [MCMCPar.reduced_seq_length];
			for( int col = 0; col < MCMCPar.reduced_seq_length; col++ )
				Reduced_Seq[row][col] = new double[MCMCPar.np2];
		}
	}
	// make 2dArray lCR pointer to pass back and forth from functions
	double **lCR;
	lCR = double_matrix( 1, MCMCPar.nCR );
	// make 2dArray lCRnew pointer to pass back and forth from functions
	double **lCRnew;
	lCRnew = double_matrix( 1, MCMCPar.nCR );
	// make 2dArray log_p pointer to pass back and forth from functions
	double **pCR;
	pCR = double_matrix( 1, MCMCPar.nCR );
	// make 1dArray delta_tot pointer to pass back and forth from functions
	double *delta_tot;
	delta_tot = new double [MCMCPar.nCR];
	// make 2dArray xnew pointer to pass back and forth from functions
	double **xnew;
	xnew = new double* [MCMCPar.seq];
	for( int row = 0; row < MCMCPar.seq; row++ )
		xnew[row] = new double[MCMCPar.n];
	// make 2dArray xold pointer to pass back and forth from functions
	double **xold;
	xold = new double* [MCMCPar.seq];
	for( int row = 0; row < MCMCPar.seq; row++ )
		xold[row] = new double[MCMCPar.n];
	// make 2dArray p pointer to pass back and forth from functions
	double *p;
	p = new double[MCMCPar.seq];
	// make 2dArray log_p pointer to pass back and forth from functions
	double *log_p;
	log_p = new double[MCMCPar.seq];
	// make 2dArray p_xold pointer to pass back and forth from functions
	double *p_xold;
	p_xold = new double[MCMCPar.seq];
	// make 2dArray log_p_xold pointer to pass back and forth from functions
	double *log_p_xold;
	log_p_xold = new double[MCMCPar.seq];
	// make 1dArray RandArray pointer to pass back and forth from functions
	int *RandArray;
	RandArray = new int [MCMCPar.nzoff]; // MCMCPar.nzoff * MCMCPar.DEpairs * MCMCPar.seq
	// make 2dArray CRS pointer to pass back and forth from functions
	double **CRS;
	CRS = new double* [MCMCPar.nCR];
	for( int row = 0; row < MCMCPar.nCR; row++ )
		CRS[row] = new double[MCMCPar.steps];
	// make 2dArray DEversion pointer to pass back and forth from functions
	int **DEversion;
	DEversion = new int* [MCMCPar.seq];
	for( int row = 0; row < MCMCPar.seq; row++ )
		DEversion[row] = new int[MCMCPar.DEpairs];
	// make 1dArray alpha_s pointer to pass back and forth from functions
	double *alpha_s;
	alpha_s = new double [MCMCPar.seq];
	// make 2dArray xnew pointer to pass back and forth from functions
	double *p_xnew;
	p_xnew = new double[MCMCPar.seq];
	// make 2dArray log_p_xnew pointer to pass back and forth from functions
	double *log_p_xnew;
	log_p_xnew = new double[MCMCPar.seq];
	// make 1dArray accept pointer to pass back and forth from functions
	int *accept;
	accept = new int [MCMCPar.seq];
	// make 1dArray R_stat pointer to pass back and forth from functions
	double *R_stat;
	R_stat = new double [MCMCPar.n];
	// make 2dArray Table_JumpRate pointer to pass back and forth from functions
	double **Table_JumpRate;
	Table_JumpRate = new double* [MCMCPar.n];
	for( int row = 0; row < MCMCPar.n; row++ )
		Table_JumpRate[row] = new double[MCMCPar.DEpairs];
	double **Zee;
	Zee = double_matrix( MCMCPar.nzee, MCMCPar.np2 );
	// output arrays
	...
	dream_zs( MCMCPar, ParRange, Measurement, ModelName, od, option, Reduced_Seq, X, Zee, output, Zinit,
			   ...
			   double **lCR, double *delta_tot, double *R_stat, double **lCRnew )
{
	...
	InitVariables( MCMCPar, nEvaluations, output_teller, reduced_seq_teller, Zee, Table_JumpRate, Reduced_Seq,
				   iloc, output, flag, Sequences, pCR, CRS, lCR, lCRnew );
	// Step 1: Sample MCMCPar.m0 points in the parameter space and store in Z
	if( strcmp( MCMCPar.InitPopulation.c_str(), "LHS_BASED" ) == 0 )
	...
	comp_likelihood( X2, MCMCPar, Measurement, ModelName, od, option, ModPred, p, log_p );
	...
	cout << endl;
	outfile << endl;
	if( strcmp( MCMCPar.save_in_file.c_str(), "Yes" ) == 0 )
	{
		ofstream outFile;
		char filename[100];
		for( int k = 0; k < MCMCPar.seq; k++ )
	...
	while( nEvaluations < MCMCPar.ndraw ) // Main Loop
	{
		...
		{
			// Initialize teller
			if( strcmp( MCMCPar.reduced_sample_collection.c_str(), "Yes" ) == 0 ) reduced_seq_teller++;
			...
			comp_likelihood( xnew, MCMCPar, Measurement, ModelName, od, option, ModPred, p_xnew, log_p_xnew );
			...
			if( strcmp( MCMCPar.ppCR.c_str(), "Update" ) == 0 )
			{
		...
		else if( strcmp( MCMCPar.reduced_sample_collection.c_str(), "Yes" ) == 0 )
		{
			...
			for( int col = 0; col < MCMCPar.n; col++ )
				output.R_stat[output_teller][col] = R_stat[col];
		if( strcmp( MCMCPar.save_in_memory.c_str(), "Yes" ) == 0 || strcmp( MCMCPar.reduced_sample_collection.c_str(), "Yes" ) == 0 ) // If GelmanCum() only no output needed
		{
...
void InitVariables( MCMC &MCMCPar, int &nEval, int &output_teller, int &reduced_seq_teller, double **Zee, double **Table_JumpRate, double ***Reduced_Seq,
					int &iloc, out &output, double &flag, double ***Sequences, double **pCR, double **CRS, double **lCR, double **lCRnew )
{
...
void offde( double **xold, double **Zoff, MCMC MCMCPar, string Update, double **Table_JumpRate, Range ParRange,
			 ...
			 double *alpha_s, Measure Measurement, MCMC MCMCPar, double option, double **newgen, int *accept )
{
	...
	if( option == 2 || option == 4 ) // Lnp probability evaluation
		for( int row = 0; row < MCMCPar.seq; row++ )
			alpha[row] = exp( p_xnew[row] - p_xold[row] );
