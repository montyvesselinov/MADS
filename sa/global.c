...
int sa_saltelli( struct opt_data *op )
{
	...
	if( ( gs.D_hat_a = ( double * ) malloc( op->pd->nOptParam * sizeof( double ) ) ) == NULL )
	{ tprintf( "Not enough memory!\n" ); return( 0 ); }
	// Vector of variances for total component contribution
	if( ( gs.D_hat_b = ( double * ) malloc( op->pd->nOptParam * sizeof( double ) ) ) == NULL )
	{ tprintf( "Not enough memory!\n" ); return( 0 ); }
	...
	for( i = 0; i < op->pd->nOptParam; i++ )
	{
		...
		gs.D_hat_a[i] = ( fhat2 / n_sub ) - pow( gs.f_hat_0, 2 );
		tprintf( "hat{D}_%d = %g\n", i, gs.D_hat_a[i] );
		gs.D_hat_a[i] = gsl_stats_mean( phis_half, 1, n_sub ) - pow( gs.f_hat_0, 2 );
		tprintf( "hat{D}_%d = %g\n", i, gs.D_hat_a[i] );
		gs.D_hat_a[i] = gsl_stats_covariance_m( gs.f_a, 1, gs.fmat_a[i], 1, n_sub, gs.f_hat_0, gs.f_hat_0 );
		tprintf( "hat{D}_%d = %g\n", i, gs.D_hat_a[i] );
		var_sorted( gs.f_a, gs.fmat_a[i], n_sub, gs.f_hat_0, gs.ep, &gs.D_hat_a[i] );
		...
		gs.D_hat_b[i] = ( fhat2 / n_sub ) - pow( gs.f_hat_0, 2 );
		tprintf( "hat{D}_n%d = %g\n", i, gs.D_hat_b[i] );
		gs.D_hat_b[i] = gsl_stats_mean( phis_half, 1, n_sub ) - pow( gs.f_hat_0, 2 );
		tprintf( "hat{D}_n%d = %g\n", i, gs.D_hat_b[i] );
		gs.D_hat_b[i] = gsl_stats_covariance_m( gs.f_a, 1, gs.fmat_b[i], 1, n_sub, gs.f_hat_0, gs.f_hat_0 );
		tprintf( "hat{D}_n%d = %g\n", i, gs.D_hat_b[i] );
		var_sorted( gs.f_a, gs.fmat_b[i], n_sub, gs.f_hat_0, gs.ep, &gs.D_hat_b[i] );
		tprintf( "hat{D}_n%d = %g\n", i, gs.D_hat_b[i] );
		//gs.D_hat_b[i] = ( fhat2 / n_sub ) - pow( gs.f_hat_0, 2 );
	}
	// Print sensitivity indices
	...
	for( i = 0; i < op->pd->nOptParam; i++ ) tprintf( "%d %g %g\n", i + 1, gs.D_hat_a[i] / gs.D_hat_t, 1 - ( gs.D_hat_b[i] / gs.D_hat_t ) );
	tprintf( "\n" );
	free( opt_params ); free( phis_half ); free( gs.f_a ); free( gs.f_b ); free( gs.D_hat_a ); free( gs.D_hat_b );
	free_matrix( ( void ** ) gs.var_a_lhs, n_sub );
	free_matrix( ( void ** ) gs.var_b_lhs, n_sub );
