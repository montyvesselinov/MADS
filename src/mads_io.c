...
int parse_cmd( char *buf, struct calc_data *cd )
{
	...
	for( word = strtok( buf, sep ); word; word = strtok( NULL, sep ) )
	{
		...
		if( !strncasecmp( word, "posix", 5 ) ) { w = 1; cd->posix = true; cd->bin_restart = true; sscanf( word, "posix=%d", &cd->num_proc ); if( cd->num_proc <= 0 ) cd->num_proc = 0; };
	...
	else if( strncasecmp( cd->opt_method, "lm", 2 ) == 0 )
	{
		...
		if( cd->problem_type == CALIBRATE && cd->analysis_type == SIMPLE && cd->nretries <= 1 && !( fabs( cd->obsstep ) > DBL_EPSILON ) )
		{
			if( cd->ldebug == 0 ) cd->ldebug = 1;
			if( cd->lm_eigen == 0 ) cd->lm_eigen = 1;
		}
	}
