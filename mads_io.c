...
int parse_cmd( char *buf, struct calc_data *cd )
{
	...
	for( word = strtok( buf, sep ); word; word = strtok( NULL, sep ) )
	{
		...
		if( strcasestr( word, "igrn" ) ) { w = 1; cd->problem_type = CALIBRATE; cd->calib_type = IGRND; }
		if( strcasestr( word, "iglh" ) ) { w = 1; cd->problem_type = CALIBRATE; cd->calib_type = IGRND; }
		if( strcasestr( word, "igpd" ) ) { w = 1; cd->problem_type = CALIBRATE; cd->calib_type = IGPD; }
		...
		if( strcasestr( word, "nosi" ) ) { w = 1; cd->sintrans = 0; }
...
int load_problem( char *filename, int argn, char *argv[], struct opt_data *op, struct calc_data *cd, struct param_data *pd, struct obs_data *od, struct well_data *wd, struct grid_data *gd, struct extrn_data *ed )
{
	...
	*/
	...
	if( cd->solution_type == EXTERNAL )
	{
		...
		else if( cd->oweight == 2 ) { if( abs(( *od ).obs_target[i] ) > DBL_EPSILON )( *od ).obs_weight[i] = ( double ) 1.0 / ( *od ).obs_target[i]; else( *od ).obs_weight[i] = HUGE; }
		if( cd->debug ) printf( "\n" );
	...
	for( i = 0; i < ( *wd ).nW; i++ )
	{
		...
		for( j = 0; j < ( *wd ).nWellObs[i]; j++ )
		{
			...
			else if( cd->oweight == 2 ) { if( abs(( *wd ).obs_target[i][j] ) > DBL_EPSILON )( *wd ).obs_weight[i][j] = ( double ) 1.0 / ( *wd ).obs_target[i][j]; else( *wd ).obs_weight[i][j] = HUGE; }
...
//	char buf[100];
	...
	*/
	...
	if( cd->ntribe > 1 ) fprintf( outfile, " ntribe=%d", cd->ntribe );
	else if( cd->ntribe < 0 ) fprintf( outfile, " ntribe" );
