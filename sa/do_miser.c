...
int
do_gsl_monte_miser_integrate (gsl_monte_function * f,
                           const double xl[], const double xu[],
                           size_t dim, size_t calls,
                           gsl_rng * r,
                           do_gsl_monte_miser_state * state,
                           double *result, double *abserr)
{
...

  {
    ...
    status = do_gsl_monte_miser_integrate (f, xl, xu_tmp,
                                        dim, calls_l, r, state,
                                        &res_l, &err_l);
    free (xu_tmp);
...

  {
    ...
    status = do_gsl_monte_miser_integrate (f, xl_tmp, xu,
                                        dim, calls_r, r, state,
                                        &res_r, &err_r);
    free (xl_tmp);
...
do_gsl_monte_miser_alloc (size_t dim)
{
  do_gsl_monte_miser_state *s =
    (do_gsl_monte_miser_state *) malloc (sizeof (do_gsl_monte_miser_state));
  ...
  do_gsl_monte_miser_init (s);

  return s;
}

int
do_gsl_monte_miser_init (do_gsl_monte_miser_state * s)
{
...
void
do_gsl_monte_miser_free (do_gsl_monte_miser_state * s)
{
  if( s == NULL ) return;
  free (s->hits_r);
  free (s->hits_l);
...
void
do_gsl_monte_miser_params_get (const do_gsl_monte_miser_state * s, do_gsl_monte_miser_params * p)
{
...
void
do_gsl_monte_miser_params_set (do_gsl_monte_miser_state * s, const do_gsl_monte_miser_params * p)
{
