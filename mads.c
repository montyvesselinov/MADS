...
int main( int argn, char *argv[] )
{
	...
	if( argn < 2 )
	{
		printf( "ERROR: argument(s) needed\n\n" );
		printf( "USAGE: %s problem_name [options]                   OR\n", argv[0] );
		printf( "       %s problem_input_file [options]\n\n", argv[0] );
		printf( "problem_name:         root name of the solved problem; problem_input_file named problem_name.mads is expected\n" );
		printf( "problem_input_file:   problem input file in MADS format\n\n" );
		...
		printf( "   igrnd              - sequential calibration using a set of random initial values (number of realizations defined by real=X)\n" );
		printf( "   igpd               - sequential calibration using a set of discretized initial values (discretization defined in the input file)\n" );
		printf( "   ppsd               - sequential calibration using partial parameter space discretization (PPSD) method\n" );
		...
		printf( "   tribe=[integer]    - number of APSO tribes [default tribe=10+sqrt(Number_of_params)]\n" );
		...
		printf( "   smp=olhs           - Optimal Latin Hyper Cube sampling [default] (if real = 1 RANDOM; if real > 1 & <= 500 IDLHS; if real > 500 LHS)\n" );
		...
		printf( "   seed=[integer]     - random seed value [default - randomly generated]\n" );
		...
		printf( "                          18: Eason 2D\n" );
		printf( "   dim=[integer]      - dimensionally of parameter space for the test problem (fixed for some of the problems) [default=2]\n" );
		...
		printf( "   ldebug=[0-2]       - Levenberg-Marquardt optimization debugging [default ldebug=0]\n" );
		printf( "   pdebug=[0-2]       - Particle Swarm optimization debugging [default pdebug=0]\n" );
		printf( "   mdebug=[0-2]       - Random sampling debugging [default mdebug=0]\n" );
...
//
	if( cd.problem_type == CALIBRATE && cd.calib_type == IGRND ) /* Calibration analysis using random initial guessed */
	{
		...
		printf( "Random sampling (vars %d; realizations %d) using ", npar, cd.nreal );
		...
		{
			cd.eval = 0;
			fprintf( out, "%d : init var", count + 1 ); // counter
...
//
	...
	{
		printf( "\nSEQUENTIAL CALIBRATION using discretized initial guesses for model parameers:\n" );
		if( pd.nFlgParam == 0 )
			printf( "WARNING: No flagged parameters! Discretization of the initial guesses cannot be performed!\n" );
		if( pd.nOptParam == 0 )
			printf( "WARNING: No parameters to optimize! Forward runs will be performed instead.\n" );
...
//
	if( cd.problem_type == CALIBRATE && cd.calib_type == SIMPLE ) /* Inverse analysis */
	{
		...
		if( status == 0 ) { printf( "ERROR: Optimization did not start! Optimization method mismatch!\n" ); exit( 0 ); }
		...
		for( i = 0; i < pd.nParam; i++ )
			if( pd.var_opt[i] >= 1 )
			{
				if( pd.var_log[i] == 0 ) printf( "%s %g\n", pd.var_id[i], pd.var[i] );
				else printf( "%s %g\n", pd.var_id[i], pow( 10, pd.var[i] ) );
				if( pd.var_log[i] == 0 ) fprintf( out, "%s %g\n", pd.var_id[i], pd.var[i] );
				else fprintf( out, "%s %g\n", pd.var_id[i], pow( 10, pd.var[i] ) );
			}
		if( pd.nParam > pd.nOptParam )
		{
			printf( "\nFixed parameters:\n" );
			fprintf( out, "\nFixed parameters:\n" );
			for( i = 0; i < pd.nParam; i++ )
		...
		if( status_all )
		{
			printf( "All the predictions are within calibration ranges!\n" );
			fprintf( out, "All the predictions are within calibration ranges!\n" );
		}
		else
		{
			printf( "At least one of the predictions is outside calibration ranges!\n" );
			fprintf( out, "At least one of the predictions is outside calibration ranges!\n" );
		}
		if( od.nObs < od.nTObs )
		{
...
//		gsl_qrng *q = gsl_qrng_alloc( gsl_qrng_sobol, pd.nOptParam );
		...
		printf( "Random sampling set 1 (vars %d; realizations %d) using ", pd.nOptParam, cd.nreal );
		sampling( pd.nOptParam, n_sub, &cd.seed, var_a_lhs, &op );
		printf( "done.\n" );
		printf( "Random sampling set 2 (vars %d; realizations %d) using ", pd.nOptParam, cd.nreal );
...
int optimize_pso( struct opt_data *op )
{
	...
	if( op->cd->leigen ) eigen( op, NULL, NULL );
	return( 1 );
}
...
int optimize_lm( struct opt_data *op )
{
	...
	if( op->cd->paranoid )
	{
		...
		printf( "Random sampling for paranoid optimization (vars %d; realizations %d) using ", npar, op->cd->nretries );
	...
	do // BEGIN Paranoid loop
	{
		...
		else // DEFAULT LevMar version of LM
		{
			...
			while( op->cd->maxeval > op->cd->eval )
			{
				...
				maxiter *= 10; // About 10 lambda searches per iteration
...
/*		op->pd->var_current_gsl = gsl_vector_alloc( op->pd->nOptParam );
	...
	sprintf( filename, "%s.jacobian", op->root ); // Save Jacobian
	out = Fwrite( filename );
	...
	fclose( out );
	printf( "Jacobian matrix stored (%s.jacobian)\n",  op->root );
	if( op->cd->problem_type == EIGEN || op->cd->leigen )
	{
		...
		sprintf( filename, "%s.covariance", op->root );
		out = Fwrite( filename );
		for( i = 0; i < op->pd->nOptParam; i++ )
		{
		...
		fclose( out );
		printf( "Covariance matrix stored (%s.covariance)\n",  op->root );
		...
		if( ier == 0 )
		{
			...
			sprintf( filename, "%s.correlation", op->root );
			out = Fwrite( filename );
			for( i = 0; i < op->pd->nOptParam; i++ )
			{
				fprintf( out, "%-25s :", op->pd->var_id[op->pd->var_index[i]] );
			...
			fclose( out );
			printf( "Correlation matrix stored (%s.correlation)\n",  op->root );
			...
			sprintf( filename, "%s.eigen", op->root );
			out = Fwrite( filename );
			for( i = 0; i < op->pd->nOptParam; i++ )
			{
				fprintf( out, "%-25s :", op->pd->var_id[op->pd->var_index[i]] );
			...
			fprintf( out, "\n" );
			fclose( out );
			printf( "Eigen vactors and eigen values stored (%s.eigen)\n",  op->root );
		}
		else
...
void save_results( char *label, struct opt_data *op, struct grid_data *gd )
{
	...
	if( op->s > 0 && op->cd->nreal > 1 ) sprintf( filename, "%s_%08d", filename, op->s );
	strcpy( f, filename );
