...
int sa_sobol( struct opt_data *op )
{
	...
	if( op->cd->mdebug )
	{
		...
		if( Ftest( filename ) == 0 ) { sprintf( buf, "%s \"mv %s.sobol.zip %s.sobol_%s.zip %s\"", BASH, op->root, op->root, Fdatetime( filename, 0 ), QUIET ); system( buf ); }
		sprintf( buf, "%s \"zip -m %s.sobol.zip %s.sobol_set_* %s \"", BASH, op->root, op->root, QUIET ); system( buf );
		sprintf( buf, "%s \"mv %s.sobol.zip %s.sobol_%s.zip %s \"", BASH, op->root, op->root, Fdatetime( filename, 0 ), QUIET ); system( buf );
	...
	if( op->cd->mdebug > 1 )
	{
		...
		if( Ftest( filename ) == 0 ) { sprintf( buf, "%s \"mv %s %s.sobol_%s.results %s \"", BASH, filename, op->root, Fdatetime( filename, 0 ), QUIET ); system( buf ); }
		out = Fwrite( filename );
	}
